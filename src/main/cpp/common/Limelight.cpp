...
void Limelight::RobotPeriodic(const RobotData &robotData, LimelightData &limelightData, VisionLookup &visionLookup)
{
    ...
    if (robotData.shooterData.currentTurretAngle > turretBackwardsDegrees_CCW + 10 || (robotData.shooterData.currentTurretAngle > turretBackwardsDegrees_C + 10 && robotData.shooterData.currentTurretAngle < turretBackwardsDegrees_C + 35))
    {
        if ((std::abs(limelightData.turretDifference + 2)) < std::min((180/pi)*std::atan(9/limelightData.distanceOffset), (double)4))
        {
...
double Limelight::interpolationVel(LimelightData &limelightData, const RobotData &robotData)
{
    ...
    double tempTurretAngle = 0;
    double additionalRPM = 0;
    ...
    { //on the right side of the turret   
        ...
        tempTurretAngle = turretAngle - turretBackwardsDegrees_C;

        if (tempTurretAngle < 90)
        {
            additionalRPM = 50 + (0.5*(tempTurretAngle)) - (0.001235 * pow(tempTurretAngle, 2));
        }
        else
        {
            additionalRPM = (forwardVelOffset/180) * tempTurretAngle;
        }
    ...
    { //left side of the robot
        ...
        if (turretAngle < turretBackwardsDegrees_C)
        {
            turretAngle += (turretBackwardsDegrees_CCW - turretBackwardsDegrees_C);
        }

        tempTurretAngle = turretBackwardsDegrees_CCW - turretAngle;
    ...
    return additionalRPM + velBackwards;
}
